...
tlgm_token = '1956324864:AAEKb3ZHknfm4E3XOaICS9YF0EXcfF26P98'                   
tlgm_id    = '1059240009'                                                         
...
while True : 
     for ticker in tickers :
        ...
        orderbook = pd.DataFrame(bids_asks)
        asks = orderbook['ask_size'].sum()
        bids = orderbook['bid_size'].sum()
        ab_rate = round(asks/bids,2)
        ...
        # 차트데이터 가져오기
        ######################################################################
        ticker_df_1   = pyupbit.get_ohlcv(ticker) 
        open_price_1  = ticker_df_1[  'open']
        high_price_1  = ticker_df_1[  'high']
        low_price_1   = ticker_df_1[   'low']
        close_price_1 = ticker_df_1[ 'close']
        volume_1      = ticker_df_1['volume']
        ...
        # 이동평균 구하기
        ###################################################################### 
        ma2_1  = ticker_df_1[ 'ma2'] =  close_price_1.rolling(2).mean()
        ma5_1  = ticker_df_1[ 'ma5'] =  close_price_1.rolling(5).mean()
        ...
        if (min_low_69[-1] < min_low_69[-2]) :
            ...
            operation_df.loc[(operation_df.ticker == ticker),     'resist_line'] = max_high_7[-1]
            operation_df.loc[(operation_df.ticker == ticker),        'out_time'] = time.strftime('%H:%M:%S')
        ...
        ######################################################################
        # 조건
        if (ma5_1[-1] > ma5_1[-2] and 
            get_in == 0) :
            if (max_high_69[-1] > max_high_69[-2] or 
                ma2[-1] > resist_line ) : 
                operation_df.loc[(operation_df.ticker == ticker),      'get_in'] = 1
                operation_df.loc[(operation_df.ticker == ticker),     'in_time'] = time.strftime('%H:%M:%S')
        ...
        print(time.strftime('%H:%M:%S'), "", 
              "", ticker.ljust(10),
              " 5:",1 if ma5_1[-1] > ma5_1[-2] else 0,
              " A/B:", str(ab_rate).ljust(5),
              " I:", 1 if get_in > 0 else 0,
              " O:", 1 if get_out > 0 else 0,
              " R:", resist_line,
              )
